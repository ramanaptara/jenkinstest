...
if (!empty($_GET)) {
	...
	if ($timestamp > 0 && $diff <= $timeout) { // less than N minutes passed since this link was created, so it's still ok
		...
		if(empty($urecords->id)){
			$urecords = $DB->get_record('user', array('email'=>$email));
		}
		...
		if($user_role=='administrator'){
			...
			if (!empty($urecords->id)) { 
				$user = get_complete_user_data('id', $urecords->id);			
			} else { 
				redirect($CFG->portal_url.'/my-account/');
			}
				...
				foreach ($ids as $course) {
					if ($DB->record_exists('course', array('idnumber'=>$course))) {
						...
						if (get_config('auth/wp2moodle', 'redirectnoenrol') !== 'yes') {
							if (!enrol_try_internal_enrol($courserow->id, $user->id, $UsrRole)) {
								continue;
							}
						}
